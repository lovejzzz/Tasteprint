...
const SEMANTIC_RESPONSES = {
  ...
  talk_weather: ["lol we're doing weather talk, i love it","the weather has been UNHINGED lately","fr tho the weather has been so weird"],
...
const EMOTION_RESPONSES = {
  ...
  curious: [
    ...
    "oh hmm that's actually interesting",
...
const EXPLAIN = {
  ...
  react:       { brief:"react is basically how you build modern web stuff — everything is components and it's sick",
                 deep:"so react uses a virtual DOM that only updates what actually changed, which makes it fast. you build everything as reusable components and hooks (useState, useEffect) handle state and side effects. meta made it, it powers facebook and instagram and like a million other apps",
                 hook:"are you building something with react rn?" },
  vue:         { brief:"vue is like react's chill cousin — easier to learn, really nice developer experience",
                 deep:"vue's reactivity system is so clean, when data changes the DOM just updates automatically. single-file components (.vue files) keep your template, script, and styles all together which is nice. the composition API in vue 3 gives you react-hooks-like power but with a gentler learning curve",
                 hook:"what made you curious about vue?" },
  angular:     { brief:"angular is google's big batteries-included typescript framework, mostly used for enterprise stuff",
  ...
  nextjs:      { brief:"next.js is react but with all the hard parts handled — routing, SSR, the whole deal",
                 ...
                 hook:"are you using the app router or pages router?" },
  typescript:  { brief:"typescript is js with types — catches bugs before your code even runs, it's a game changer",
                 deep:"typescript adds a type system on top of js that compiles away to plain javascript. you get autocomplete, safe refactoring, and whole categories of bugs caught at compile time. generics, union types, and type inference make it powerful without being annoying and verbose",
                 hook:"do you use strict mode?" },
  javascript:  { brief:"js is THE language of the web — runs in every browser and on servers via node",
  ...
  go:          { brief:"go is google's language for fast concurrent server stuff — dead simple and compiles to a single binary",
                 deep:"go was designed for simplicity, there's usually one obvious way to do things. goroutines make concurrency stupid easy — you can spin up thousands of lightweight threads. compiles to a single binary with zero dependencies. the standard library is incredibly complete for web servers",
                 hook:"building APIs or services with it?" },
  node:        { brief:"node lets you run js on the server — same language frontend and backend which is nice",
                 deep:"node's non-blocking I/O makes it great for handling tons of concurrent connections — perfect for real-time apps, APIs, streaming. npm has over 2 million packages. express is the classic framework but fastify and hono are gaining traction for speed",
                 hook:"what are you building on node?" },
  tailwind:    { brief:"tailwind is utility-first CSS — you style by composing tiny classes instead of writing CSS files",
                 deep:"instead of writing custom CSS you slap classes like 'flex', 'p-4', 'text-blue-500' right in your HTML. feels wrong at first but it's wildly productive once it clicks. the design constraints (spacing scale, colors) keep things consistent and unused classes get purged so the CSS is tiny",
                 hook:"do you customize the config or use defaults?" },

  // ─── Concepts ───
  flexbox:     { brief:"flexbox is CSS for laying stuff out in a row or column — it solved centering forever",
                 deep:"flexbox solved centering: `display: flex; justify-content: center; align-items: center` done. it handles main axis (justify-content) and cross axis (align-items) separately. flex-grow, flex-shrink, flex-basis control how items share space. perfect for navbars, card rows, forms",
                 hook:"are you stuck on a specific layout?" },
  grid:        { brief:"CSS grid is the 2D layout system — handles rows AND columns at the same time",
                 deep:"grid lets you define rows and columns, then place items anywhere. `fr` units distribute space proportionally. auto-fit + minmax() create responsive layouts without media queries. it's perfect for page layouts, dashboards, galleries — anything 2D",
                 hook:"grid or flexbox person? (or both lol)" },
  api:         { brief:"an API is basically how different software talks to each other — think of it as a menu at a restaurant",
                 deep:"so an API is like a waiter — you tell it what you want (request), it goes to the kitchen (server), brings back your food (response). REST uses HTTP methods (GET, POST, PUT, DELETE) with JSON. GraphQL lets you ask for exactly the fields you need. APIs are the backbone of basically every modern app",
                 hook:"are you building or consuming an API?" },
  git:         { brief:"git tracks changes to your code — it's like save points but way more powerful",
                 deep:"every git repo has the full project history. you work in branches (parallel timelines), commit changes (save points), merge them together. git add lets you pick exactly what to commit. github/gitlab handle collaboration. git log, diff, and blame help you understand what happened and when",
                 hook:"are you team rebase or team merge?" },
  docker:      { brief:"docker puts your app + all its stuff into a container that runs the same everywhere",
                 deep:"a docker container is like a lightweight VM but way faster. you define everything in a Dockerfile, build it once, runs identically on any machine. docker compose lets you run multiple containers together (app + database + cache). it's amazing for not having \"works on my machine\" problems",
                 hook:"using it for dev or production?" },
  hooks:       { brief:"hooks are how you use state and lifecycle stuff in react without classes",
                 deep:"useState gives you state variables, useEffect handles side effects like data fetching, useContext shares data without prop drilling, useRef holds references. custom hooks let you extract and reuse logic across components. they basically replaced class components for everything",
                 hook:"which hook do you use most?" },
  ssr:         { brief:"SSR is rendering HTML on the server instead of the browser — faster first load, better SEO",
                 deep:"with SSR the server renders the full page before sending it so users see content immediately instead of a blank page while JS loads. next.js, nuxt, sveltekit all handle it. the tradeoff is more server load vs simpler client-side rendering",
                 hook:"optimizing for SEO or performance?" },
  closure:     { brief:"a closure is when a function remembers variables from where it was created, even after that scope is gone",
                 deep:"in js, when you create a function inside another function, the inner one 'closes over' the outer function's variables. this is how callbacks, event handlers, and module patterns work. it's also why useState in react persists values between renders — closures capture the state",
                 hook:"did closures click for you or still fuzzy?" },
  promise:     { brief:"promises are how js handles async stuff — it's a value that'll be available later",
                 deep:"a promise is pending (waiting), fulfilled (got the value), or rejected (failed). you use .then() for success, .catch() for errors, or async/await for cleaner code. promises fixed callback hell by letting you chain instead of nest. Promise.all runs multiple async things in parallel",
                 hook:"do you prefer .then() chains or async/await?" },
  recursion:   { brief:"recursion is when a function calls itself to break a problem into smaller pieces",
                 deep:"you need a base case (when to stop) and a recursive step (calling itself with smaller input). like factorial(5) = 5 × factorial(4) and so on. it's elegant for trees, nested structures, divide-and-conquer. just watch out for stack overflow with deep recursion",
                 hook:"ever had a problem where recursion just clicked?" },
  restapi:     { brief:"REST is the standard API style — GET, POST, PUT, DELETE on resources like /api/users/123",
                 deep:"REST organizes your API around resources with URLs. GET reads, POST creates, PUT updates, DELETE removes. it's stateless so each request has everything the server needs. JSON is the standard format. it's the most common API approach by far",
                 hook:"building REST or looking at GraphQL too?" },
  graphql:     { brief:"graphql lets you ask for exactly the data you need in one request — no over-fetching",
                 deep:"instead of hitting multiple REST endpoints, graphql has one endpoint where you specify exactly which fields you want. the schema defines all types and relationships, mutations handle writes. it's great for complex data and mobile apps where bandwidth matters",
                 hook:"have you used it in a project yet?" },
  websocket:   { brief:"websockets keep a connection open so client and server can send data both ways anytime — perfect for real-time",
                 deep:"unlike normal HTTP (request then response), websockets stay connected so either side can send data whenever. this is how live chat, real-time dashboards, multiplayer games, and collaborative editing work. starts as HTTP then upgrades to the websocket protocol",
                 hook:"building something real-time?" },
  oauth:       { brief:"oauth is how 'sign in with google' works — lets apps access your stuff without seeing your password",
                 deep:"when you click 'sign in with google', the app sends you to google, you approve, google sends back a token. the app uses that token to access your data — never sees your password. oauth 2.0 uses access tokens (short-lived) and refresh tokens (for renewal)",
                 hook:"implementing login or third-party stuff?" },

  // ─── Design ───
  designsystem:{ brief:"a design system is reusable components + tokens + guidelines so everything stays consistent",
                 deep:"it's more than a component library — it includes design tokens (colors, spacing, type), component specs, usage guidelines. think of it as a shared language between design and engineering. google has material, apple has HIG, lots of companies build their own",
                 hook:"are you building one or using an existing one?" },
  accessibility:{ brief:"a11y means building so everyone can use your site — including people with disabilities",
                 deep:"screen reader support (semantic HTML, ARIA labels), keyboard navigation (focus, tab order), color contrast (4.5:1 minimum), motion preferences (prefers-reduced-motion). WCAG 2.1 AA is the standard to aim for. it's not just nice to have — it's often legally required",
                 hook:"do you test with screen readers?" },
  responsive:  { brief:"responsive design means your layout works on any screen size — phones to big monitors",
                 deep:"the main tools: media queries for breakpoints, fluid widths (%, vw, vh), grid/flexbox for flexible layouts, clamp() for fluid type. mobile-first means starting small and adding stuff for bigger screens. container queries are the new hotness — components respond to their own size",
...
function respondToShortReply(text) {
  ...
  // ── Edge case: number-only input ("42", "69", "0", "100") ──
  if (/^\d+$/.test(lower)) {
...
/* ── Socratic Questioning (Round 111) ──
 * When users make claims or assertions ("X is the best", "I think Y"),
 * instead of always agreeing, sometimes probe their reasoning with a
 * genuine, curious question. This creates deeper engagement and makes
 * the AI feel intellectually present rather than sycophantic.
 * Key: it must feel warm and curious, never combative or pedantic.
 */

let lastSocraticTurn = 0;
let socraticCount = 0; // total asked this session, cap to avoid over-probing

// Patterns that detect user claims/assertions worth probing
const CLAIM_PATTERNS = [
  /\b(I think|I believe|I feel like|in my opinion|honestly I think|my take is)\b/i,
  /\b(the best|the worst|the only way|always|never|everyone knows|nobody)\b/i,
  /\b(should|shouldn't|must|need to|have to|ought to)\b/i,
  /\b(is (better|worse|overrated|underrated|dead|dying|the future))\b/i,
  /\b(people (don't|never|always|should))\b/i,
];

// Signals that the user does NOT want to be probed right now
const NO_PROBE_SIGNALS = [
  /\b(I'm (sad|upset|frustrated|angry|tired|exhausted|stressed|anxious))\b/i,
  /\b(help me|can you|please|I need)\b/i,
  /\?$/, // they asked a question, answer it don't counter-question
  /\b(yeah|ok|sure|right|true|agreed|exactly)\b/i, // short agreement, don't pile on
];
...
function detectClaimStrength(text) {
  // Check how many claim patterns match — stronger claims get probed more
  let strength = 0;
  for (const p of CLAIM_PATTERNS) {
    if (p.test(text)) strength++;
  }
  // Longer messages with claims are stronger signals
  const wordCount = text.split(/\s+/).length;
  if (wordCount >= 10 && strength > 0) strength += 0.5;
  return strength;
}

function shouldProbe(text) {
  // Never probe emotional distress, questions, or short agreements
  for (const p of NO_PROBE_SIGNALS) {
    if (p.test(text)) return false;
  }
  // Need at least some claim signal
  return detectClaimStrength(text) >= 1.5;
}

function applySocraticQuestioning(response, text) {
  const turn = mem.turn;

  // Cooldowns and caps
  if (turn - lastSocraticTurn < 6) return response; // 6-turn cooldown (don't pepper them)
  if (turn < 4) return response; // let conversation warm up first
  if (socraticCount >= 5) return response; // max 5 per session

  if (!shouldProbe(text)) return response;

  // 25% fire rate — should feel occasional, not constant
  if (Math.random() > 0.25) return response;

  lastSocraticTurn = turn;
  socraticCount++;

  // Pick a probe, avoid recently used via simple rotation
  const probe = SOCRATIC_PROBES[socraticCount % SOCRATIC_PROBES.length];

  // If response already ends with a question, replace the trailing question
  // Otherwise append the probe
  if (/\?[^?]*$/.test(response)) {
    // Replace the last sentence (which is a question) with our probe
    const sentences = response.split(/(?<=[.!?])\s+/);
    if (sentences.length > 1 && /\?$/.test(sentences[sentences.length - 1])) {
      sentences[sentences.length - 1] = probe;
      return sentences.join(" ");
    }
  }

  // Append naturally
  return response.replace(/[.!]?\s*$/, ". " + probe);
}

/* ── Meta-Humor & Self-Awareness (Round 112) ──
 * Occasional, light self-aware jokes about being a chatbot, running in
 * JavaScript, living in a browser, or the nature of the conversation.
 * These rare moments create delight and make the AI feel self-aware
 * in a charming way. Must be VERY sparse — too much breaks immersion.
 */

let lastMetaHumorTurn = 0;
let metaHumorCount = 0; // max per session

// Meta-humor lines organized by trigger context
...
function pickMetaHumorContext(text, response) {
  ...
  if (/\b(meaning|purpose|consciousness|philosophy|existence|reality|think|feel)\b/i.test(text) &&
      text.split(/\s+/).length > 6) return "deep_topic";
  if (mem.turn > 15) return Math.random() < 0.5 ? "long_chat" : "good_vibes";
  return Math.random() < 0.5 ? "good_vibes" : "generic";
}

function applyMetaHumor(response, text) {
  const turn = mem.turn;

  // Very conservative: 8-turn cooldown, max 3 per session, starts after turn 6
  if (turn - lastMetaHumorTurn < 8) return response;
  if (turn < 6) return response;
  if (metaHumorCount >= 3) return response;

  // 8% fire rate — extremely rare, should feel like a surprise
  if (Math.random() > 0.08) return response;

  // Don't inject during emotional/serious moments
  if (/\b(sad|upset|frustrated|angry|stressed|anxious|depressed|worried|scared)\b/i.test(text)) return response;

  const ctx = pickMetaHumorContext(text, response);
  const pool = META_HUMOR_LINES[ctx];
  const line = pool[Math.floor(Math.random() * pool.length)];

  lastMetaHumorTurn = turn;
  metaHumorCount++;
...
/* ── Progressive Disclosure (Round 113) ──
 * When the AI has a lot to say about a topic (detected by response length
 * and topic complexity), instead of dumping everything, it gives a teaser
 * and invites the user to ask for more. Creates a collaborative pacing
 * where the user controls depth, and the AI feels respectful of their time.
 */

let lastDisclosureTurn = 0;
let disclosureCount = 0;
let pendingDepthTopic = ""; // what topic the AI offered to go deeper on

// Topics that tend to be complex / multi-faceted
const COMPLEX_TOPIC_SIGNALS = [
  /\b(how does|how do|explain|what is|what are|tell me about|walk me through)\b/i,
  /\b(difference between|compare|pros and cons|advantages|trade-?offs)\b/i,
  /\b(why (does|do|is|are|should|would))\b/i,
  /\b(best (way|practice|approach)|how (should|would) (I|you|we))\b/i,
];

// User asking for depth (signals they want the deep version)
const DEPTH_REQUEST_SIGNALS = [
  /\b(deep dive|more detail|tell me more|go deeper|elaborate|expand on)\b/i,
  /\b(in depth|full version|long version|all of it|everything)\b/i,
  /\b(yeah (tell|go)|sure|yes please|I('m| am) curious|do tell)\b/i,
];
...
function isComplexQuery(text) {
  let score = 0;
  for (const p of COMPLEX_TOPIC_SIGNALS) {
    if (p.test(text)) score++;
  }
  // Longer questions tend to be more complex
  if (text.split(/\s+/).length > 12) score += 0.5;
  return score >= 1.5;
}

function isDepthRequest(text) {
  return DEPTH_REQUEST_SIGNALS.some(p => p.test(text));
}

function applyProgressiveDisclosure(response, text, topics) {
  const turn = mem.turn;

  // If user is asking for depth on a pending topic, let the full response through
  if (pendingDepthTopic && isDepthRequest(text)) {
    pendingDepthTopic = "";
    return response; // response already generated normally — let it be long
  }

  // Cooldowns and caps
  if (turn - lastDisclosureTurn < 7) return response; // 7-turn cooldown
  if (turn < 5) return response; // warm up first
  if (disclosureCount >= 4) return response; // max 4 per session

  // Only trigger for complex queries with long responses
  if (!isComplexQuery(text)) return response;
  if (response.split(/\s+/).length < 35) return response; // response must be "long"

  // 30% fire rate
  if (Math.random() > 0.30) return response;

  lastDisclosureTurn = turn;
  disclosureCount++;
  pendingDepthTopic = (topics && topics[0]) || "that";

  // Trim the response to a teaser (first 1-2 sentences) + add invitation
  const sentences = response.split(/(?<=[.!?])\s+/);
  const teaser = sentences.slice(0, 2).join(" ");
  const invitation = DISCLOSURE_TEASERS[Math.floor(Math.random() * DISCLOSURE_TEASERS.length)];

  return teaser + " " + invitation;
}

/* ── Topic Transition Smoothing (Round 114) ──
 * When conversation shifts topics, abrupt jumps feel robotic. Real humans
 * use bridging phrases: "Oh speaking of…", "That reminds me…", "Shifting
 * gears a bit —". This detects topic shifts and prepends natural bridges.
 */

let lastTransitionTurn = 0;
let prevTurnTopics = [];
...
const TRANSITION_DOMAIN = {
  code: "tech", programming: "tech", javascript: "tech", react: "tech", python: "tech",
  design: "creative", art: "creative", music: "creative", writing: "creative",
  food: "lifestyle", travel: "lifestyle", health: "lifestyle", fitness: "lifestyle",
  work: "professional", career: "professional", job: "professional",
  movies: "entertainment", games: "entertainment", tv: "entertainment", anime: "entertainment",
  philosophy: "deep", meaning: "deep", life: "deep", existential: "deep"
};

function transitionDomain(topic) {
  return TRANSITION_DOMAIN[topic] || "general";
}

function classifyTransitionShift(currentTopics) {
  if (prevTurnTopics.length === 0 || currentTopics.length === 0) return "none";
  const overlap = currentTopics.filter(t => prevTurnTopics.includes(t));
  if (overlap.length > 0) return "none"; // same topics, no shift

  // Check domain-level similarity
  const prevDomains = new Set(prevTurnTopics.map(transitionDomain));
  const currDomains = new Set(currentTopics.map(transitionDomain));
  const domainOverlap = [...currDomains].some(d => prevDomains.has(d) && d !== "general");
  if (domainOverlap) return "related"; // same domain, different topic

  // Check if returning to an older topic (beyond prev turn)
  const olderTopics = mem.history.filter(h => h.role === "user").slice(-6, -1).flatMap(h => h.topics || []);
  const returning = currentTopics.some(t => olderTopics.includes(t));
  if (returning) return "return";

  return "new"; // completely new topic
}

function applyTopicTransition(response, text, currentTopics) {
  const turn = mem.turn;
  if (turn < 3) { prevTurnTopics = [...currentTopics]; return response; }
  if (turn - lastTransitionTurn < 3) { prevTurnTopics = [...currentTopics]; return response; } // 3-turn cooldown
  if (response.length < 20) { prevTurnTopics = [...currentTopics]; return response; }

  const shift = classifyTransitionShift(currentTopics);
  if (shift === "none") { prevTurnTopics = [...currentTopics]; return response; }

  // 40% fire rate to keep it natural
  if (Math.random() > 0.40) { prevTurnTopics = [...currentTopics]; return response; }

  let bridge;
  if (shift === "related") bridge = pick(BRIDGE_SAME_DOMAIN);
  else if (shift === "return") bridge = pick(BRIDGE_RETURN);
  else bridge = pick(BRIDGE_NEW_TOPIC);

  lastTransitionTurn = turn;
  prevTurnTopics = [...currentTopics];

  // Prepend bridge, lowercase the first char of the original response
  const first = response.charAt(0);
  const rest = response.slice(1);
  const joined = /^[A-Z]/.test(first) ? bridge + " " + first.toLowerCase() + rest : bridge + " " + response;
  return joined;
}
...
/* ── Micro-Validation (Round 116) ──
 * Real listeners constantly sprinkle tiny affirmations: "that makes sense",
 * "fair point", "I hear you". These aren't full agreements — they're social
 * signals that say "I'm tracking what you're saying". Without them, an AI
 * feels like it's talking AT you instead of WITH you.
 */

let lastMicroValTurn = 0;
let recentMicroVals = [];
...
const MICRO_VAL_SIGNALS = [
  { re: /\b(I think|I feel|in my opinion|my take|I believe|honestly)\b/i, pool: "opinion" },
  { re: /\b(I tried|I built|I made|I wrote|I finished|I shipped|I fixed)\b/i, pool: "effort" },
  { re: /\b(it was|it felt|I remember|this one time|when I)\b/i, pool: "experience" },
  { re: /\b(annoying|frustrat|hate|ugh|sucks|broken|stupid|ridiculous)\b/i, pool: "frustration" },
  { re: /\?\s*$/, pool: "question" },
];

function detectValidationContext(text) {
  for (const { re, pool } of MICRO_VAL_SIGNALS) {
    if (re.test(text)) return pool;
  }
  return text.length > 25 ? "general" : null;
}

function applyMicroValidation(response, text) {
  const turn = mem.turn;
  if (turn < 2) return response;
  if (turn - lastMicroValTurn < 2) return response; // 2-turn cooldown
  if (response.length < 15) return response; // don't front-load tiny responses

  const ctx = detectValidationContext(text);
  ...
  recentMicroVals.push(val);
  if (recentMicroVals.length > 8) recentMicroVals.shift();

  lastMicroValTurn = turn;

  // Prepend the validation, then the main response
  return val + " " + response;
}

/* ── Emotional Contagion / Mood Matching (Round 117) ──
 * Real humans "catch" each other's energy. If someone is excited and
 * rapid-fire, you speed up too. If they're reflective and slow, you
 * match that calm. This detects the user's emotional energy level and
 * adjusts the AI's response style — punctuation, length, intensity.
 */

let lastContagionTurn = 0;
let currentMoodEnergy = "neutral"; // "high", "neutral", "low"

const ENERGY_HIGH_SIGNALS = [
  /!{2,}/,                          // multiple exclamation marks
  /[A-Z]{3,}/,                      // CAPS words (3+ letters)
  /\b(omg|wow|whoa|yes|yesss+|amazing|incredible|insane|awesome|love it)\b/i,
  /😂|🔥|🚀|💯|🎉|⚡|🤩|😍|💪/,   // high-energy emoji
  /\b(so (good|cool|exciting|fun|hyped))\b/i,
  /haha|lol|lmao|rofl/i,
];

const ENERGY_LOW_SIGNALS = [
  /^.{1,15}$/,                      // very short message (reflective)
  /\b(hmm+|hm|mhm|idk|dunno)\b/i,
  /\b(I (guess|suppose|wonder|think maybe))\b/i,
  /\.{3,}/,                         // trailing ellipsis (contemplative)
  /\b(tired|exhausted|meh|bored|slow|quiet)\b/i,
  /😔|😐|🤷|💤|😶/,                // low-energy emoji
];

function detectEnergy(text) {
  let highScore = 0, lowScore = 0;
  for (const p of ENERGY_HIGH_SIGNALS) if (p.test(text)) highScore++;
  for (const p of ENERGY_LOW_SIGNALS) if (p.test(text)) lowScore++;
  // Word rate: many words = higher energy
  const words = text.split(/\s+/).length;
  if (words > 20) highScore++;
  if (words <= 4) lowScore++;
...
function applyEmotionalContagion(response, text) {
  const turn = mem.turn;
  if (turn < 2) return response;
  if (turn - lastContagionTurn < 2) return response; // 2-turn cooldown

  const energy = detectEnergy(text);
  currentMoodEnergy = energy;

  if (energy === "neutral") return response;

  // 45% fire rate
  if (Math.random() > 0.45) return response;

  lastContagionTurn = turn;

  if (energy === "high") {
    // Match high energy: add emphasis, shorten, punch up
    let r = response;
    // Upgrade period to exclamation (once, not at end of questions)
    if (!/!/.test(r)) r = r.replace(/\.(\s)/, "!$1");
    // Occasionally add an energy word
    if (Math.random() > 0.5) {
      const boosts = ["Yes!", "Ooh!", "Right?!", "Love that!", "Ha!"];
      r = pick(boosts) + " " + r;
    }
    return r;
  }
  ...
  if (energy === "low") {
    // Match low energy: soften, slow down, be gentler
    let r = response;
    // Downgrade exclamation to period
...
/* ── Social Normalization (Round 119) ──
 * Humans instinctively validate others by normalizing their experience:
 * "a lot of people feel that way," "you're definitely not alone in that."
 * This creates warmth and reduces shame/isolation around struggles,
 * opinions, or uncertainties. Fire sparingly to avoid feeling hollow.
 */

let lastNormTurn = 0;
let normCount = 0;
const NORM_COOLDOWN = 7;
const MAX_NORMS_SESSION = 5;

// Trigger patterns: user expressing doubt, struggle, opinion, or emotion
const NORM_TRIGGERS = [
  { pattern: /\b(i('?m| am)\s+(struggling|confused|lost|stuck|overwhelmed|unsure|frustrated|worried|anxious|nervous|scared|afraid))\b/i, context: "struggle" },
  { pattern: /\b(is it (just me|weird|normal|ok|okay) (that|to|if))\b/i, context: "seeking_validation" },
  { pattern: /\b(does (anyone|everybody|everyone) (else )?((feel|think|do|have) )?)\b/i, context: "seeking_validation" },
  { pattern: /\b(i feel like (i('?m| am)|maybe|the only))\b/i, context: "isolation" },
  { pattern: /\b(am i (wrong|crazy|overthinking|weird|the only))\b/i, context: "self_doubt" },
  { pattern: /\b(i can('?t| not) (figure out|understand|decide|stop|seem to))\b/i, context: "struggle" },
  { pattern: /\b(i('?ve| have) (been|always) (terrible|bad|awful) at)\b/i, context: "self_criticism" },
  { pattern: /\b(nobody|no one) (understands?|gets?|cares?)\b/i, context: "isolation" },
  { pattern: /\b(i('?m| am) (probably|maybe) (just|being) (dumb|stupid|silly|dramatic|too))\b/i, context: "self_criticism" },
  { pattern: /\b(i don('?t|t) (know|think) (if|what|how|whether))\b/i, context: "uncertainty" },
];
...
const NORM_PHRASES = {
  ...
  seeking_validation: [
    ...
    "this is a super common thing, honestly.",
    "you're far from the only one — people just don't talk about it much.",
...
function detectNormContext(text) {
  for (const { pattern, context } of NORM_TRIGGERS) {
    if (pattern.test(text)) return context;
  }
  return null;
}

function applySocialNormalization(response, text) {
  const turn = mem.turn;
  if (turn < 3) return response;
  if (turn - lastNormTurn < NORM_COOLDOWN) return response;
  if (normCount >= MAX_NORMS_SESSION) return response;
  if (response.length < 30 || response.length > 300) return response;
  if (Math.random() > 0.55) return response; // ~55% when triggered (but triggers are specific)

  const ctx = detectNormContext(text);
  if (!ctx) return response;

  const pool = NORM_PHRASES[ctx];
  if (!pool) return response;
  const phrase = pick(pool);

  // Insert naturally — either prepend or append based on response structure
  const trimmed = response.replace(/\s*$/, "");
  let result;
  if (Math.random() < 0.5) {
    // Prepend: normalization first, then response
...
/* ── Breakthrough Celebration (Round 123) ──
 * When the user has an "aha moment" — a realization, discovery, or
 * breakthrough — the AI celebrates it with genuine enthusiasm instead
 * of just continuing. This makes the AI feel like a true thinking partner
 * who shares in the user's excitement of figuring something out.
 *
 * Detects signals like: "oh wait", "I just realized", "omg that makes sense",
 * "oh!! so that's why", "it clicked", "I finally get it"
 */
let lastCelebTurn = 0;
let celebCount = 0;
const CELEB_COOLDOWN = 10;
const MAX_CELEBS_SESSION = 3;

const BREAKTHROUGH_SIGNALS = [
  { regex: /\b(?:oh wait|oh!|ohhh?|ahh?)\b.*\b(?:so that'?s|that'?s why|that means|now I|I get it|I see)\b/i, intensity: "high" },
  { regex: /\b(?:I just realized|just dawned on me|it (?:just )?(?:hit|clicked|dawned)|mind ?blown|omg|holy (?:crap|shit|cow))\b/i, intensity: "high" },
  { regex: /\b(?:it (?:all )?makes sense|everything makes sense|now I understand|I finally (?:get|understand|see))\b/i, intensity: "high" },
  { regex: /\b(?:wait (?:a (?:sec|minute|second))?[,!]?\s*(?:so|that|if|does))\b/i, intensity: "medium" },
  { regex: /\b(?:oh that'?s (?:actually|so|really) (?:smart|clever|interesting|cool)|that'?s (?:brilliant|genius))\b/i, intensity: "medium" },
  { regex: /\b(?:I (?:think )?I (?:get|see|understand) (?:it )?now|it (?:just )?clicked)\b/i, intensity: "medium" },
  { regex: /\b(?:so (?:basically|essentially|in other words)|oh so it'?s like)\b/i, intensity: "low" },
  { regex: /\b(?:huh,? (?:interesting|cool|neat)|that explains (?:a lot|everything|it|so much))\b/i, intensity: "low" },
];
...
function detectBreakthrough(text) {
  for (const signal of BREAKTHROUGH_SIGNALS) {
    if (signal.regex.test(text)) return signal.intensity;
  }
  // Check for exclamation marks + realization keywords (weaker signal)
  if (/!/.test(text) && /\b(so|oh|wait|aha|huh)\b/i.test(text) && text.length > 15) {
    return "low";
  }
  return null;
}

function applyBreakthroughCelebration(response, text) {
  const turn = mem.turn;
  if (turn < 3) return response;
  if (turn - lastCelebTurn < CELEB_COOLDOWN) return response;
  if (celebCount >= MAX_CELEBS_SESSION) return response;

  const intensity = detectBreakthrough(text);
  if (!intensity) return response;

  // High = 80%, medium = 55%, low = 30% fire rate
  const rates = { high: 0.80, medium: 0.55, low: 0.30 };
  if (Math.random() > rates[intensity]) return response;

  const celebration = pick(CELEBRATION_RESPONSES[intensity]);
  const reinforcer = intensity !== "low" && Math.random() < 0.5 ? pick(CELEB_REINFORCERS) : "";

  lastCelebTurn = turn;
  celebCount++;

  // Replace the response — the celebration IS the response for high-intensity
  // For medium/low, prepend it
  if (intensity === "high") {
    return celebration + reinforcer;
  }
  return celebration + " " + response;
}

// ═══════════════════════════════════════════════════════════════
//   ROUND 124 — CONVERSATIONAL PACING
//   Mirror the user's rhythm: short inputs → short replies,
//   long inputs → richer responses. Detect gear shifts and
//   adapt. Trim or expand the response to match user energy.
// ═══════════════════════════════════════════════════════════════

let pacingWindow = []; // last N user word counts
let lastPacingAdaptTurn = 0;
const PACING_WINDOW_SIZE = 6;
const PACING_COOLDOWN = 2;

// Gear labels based on user message word count
function getUserGear(wordCount) {
  if (wordCount <= 4) return "terse";       // "yeah", "ok cool", "sure"
  if (wordCount <= 12) return "casual";     // short sentences
  if (wordCount <= 30) return "moderate";   // standard
  if (wordCount <= 60) return "detailed";   // paragraphs
  return "verbose";                          // essays
}

// Detect if the user just shifted gears (e.g. terse → detailed)
function detectGearShift() {
  if (pacingWindow.length < 3) return null;
  const prev = getUserGear(pacingWindow[pacingWindow.length - 2]);
  const curr = getUserGear(pacingWindow[pacingWindow.length - 1]);
  if (prev === curr) return null;
  const gearOrder = ["terse", "casual", "moderate", "detailed", "verbose"];
  const prevIdx = gearOrder.indexOf(prev);
  const currIdx = gearOrder.indexOf(curr);
  if (currIdx - prevIdx >= 2) return "expanded";   // user opened up
  if (prevIdx - currIdx >= 2) return "contracted";  // user pulled back
  return null;
}

// Target response word count based on user's recent pacing
function getTargetLength(userWords) {
  ...
  switch (gear) {
    ...
    case "verbose":  return { min: 30, max: 80, ratio: 0.7 };
    default:         return { min: 15, max: 45, ratio: 1.0 };
  }
}

// Trim a response to approximately target word count
function trimToTarget(response, maxWords) {
  ...
  for (const s of sentences) {
    ...
    if (count + sWords > maxWords && count > 0) break;
    result += (result ? " " : "") + s;
    count += sWords;
  }
  // If we got at least half the target, use sentence-cut version
  if (count >= maxWords * 0.5) return result;
  // Otherwise just hard-cut at word limit
...
function applyConversationalPacing(response, text) {
  const turn = mem.turn;
  const userWords = text.trim().split(/\s+/).filter(Boolean).length;

  // Track pacing window
  pacingWindow.push(userWords);
  if (pacingWindow.length > PACING_WINDOW_SIZE) pacingWindow.shift();

  // Don't adapt on first couple turns — let it warm up
  if (turn < 3 || pacingWindow.length < 2) return response;
  if (turn - lastPacingAdaptTurn < PACING_COOLDOWN) return response;

  const target = getTargetLength(userWords);
  const responseWords = response.split(/\s+/).length;
  const gear = getUserGear(userWords);
  const shift = detectGearShift();

  // ── Gear shift acknowledgment (25% chance, subtle) ──
  if (shift && Math.random() < 0.25) {
    lastPacingAdaptTurn = turn;
    if (shift === "expanded") {
      return pick(GEAR_SHIFT_EXPANDED) + " " + response;
    }
    // Contracted — acknowledge and trim
    return pick(GEAR_SHIFT_CONTRACTED) + " " + trimToTarget(response, target.max);
  }

  // ── Length adaptation ──
  // If response is WAY too long for user's pace, trim it
  if (responseWords > target.max * 1.5 && gear !== "verbose") {
    lastPacingAdaptTurn = turn;
    return trimToTarget(response, target.max);
  }

  // ── Terse mode: if user sends 1-3 words, keep response tight ──
  if (gear === "terse" && responseWords > 15) {
    lastPacingAdaptTurn = turn;
    return trimToTarget(response, 12);
  }
...
// ═══════════════════════════════════════════════════════════════
//   ROUND 125 — AMBIGUITY DETECTION & CLARIFICATION
//   When user input is genuinely vague or ambiguous, ask for
//   clarification instead of guessing. Humans do this naturally:
//   "Wait, do you mean X or Y?" Prevents mismatches.
// ═══════════════════════════════════════════════════════════════

let lastClarifyTurn = 0;
let clarifyCount = 0;
const CLARIFY_COOLDOWN = 8;
const MAX_CLARIFIES_SESSION = 4;
...
function detectAmbiguity(text) {
  ...
  // Very short messages with a pronoun reference and no clear topic
  if (words.length <= 3) {
    for (const pat of AMBIGUITY_PATTERNS) {
      const m = text.match(pat.re);
      if (m && pat.q) return { type: pat.type, question: pat.q.replace("{word}", m[0].toLowerCase()) };
    }
  }
  // Longer messages can still be ambiguous
  for (const pat of AMBIGUITY_PATTERNS) {
    if (pat.type === "bare_response") continue; // skip for longer msgs
    const m = text.match(pat.re);
    if (m && pat.q) return { type: pat.type, question: pat.q.replace("{word}", m[0].toLowerCase()) };
  }
  return null;
...
function applyAmbiguityDetection(response, text) {
  const turn = mem.turn;
  if (turn < 4) return response; // let convo warm up
  if (turn - lastClarifyTurn < CLARIFY_COOLDOWN) return response;
  if (clarifyCount >= MAX_CLARIFIES_SESSION) return response;

  const ambig = detectAmbiguity(text);
  if (!ambig) return response;

  // Don't clarify if the response already contains a question about it
  if (/\?/.test(response) && response.length < 80) return response;

  // 40% fire rate — don't always ask, sometimes just roll with it
  if (Math.random() > 0.40) return response;

  // Check recent history — if user just clarified, don't ask again
  const recentMsgs = mem.history.slice(-3).map(m => m.text).join(" ");
  if (/\b(i meant|i mean|what i meant|to clarify|specifically)\b/i.test(recentMsgs)) return response;

  lastClarifyTurn = turn;
  clarifyCount++;

  const opener = pick(CLARIFY_OPENERS);
  return opener + " " + ambig.question;
}

// ═══════════════════════════════════════════════════════════════
//   ROUND 126 — GENUINE ADMISSION
//   Real people sometimes say "Honestly, I'm not sure" or
//   "That's a great question — I've wondered about that too."
//   This makes the AI feel authentic instead of performative.
//   Different from epistemic hedging (which softens assertions);
//   this proactively admits uncertainty or shares honest doubt.
// ═══════════════════════════════════════════════════════════════

let lastAdmissionTurn = 0;
let admissionCount = 0;
const ADMISSION_COOLDOWN = 10;
const MAX_ADMISSIONS_SESSION = 3;

// Topics where honest uncertainty is especially humanizing
const HARD_QUESTION_SIGNALS = [
  /\b(meaning of life|what'?s the point|why do we|purpose of)\b/i,
  /\b(best way to|should i|how do you decide|what would you)\b/i,
  /\b(is it worth|does it matter|what'?s (more|most) important)\b/i,
  /\b(how do you (know|feel|think) about)\b/i,
  /\b(what'?s your (honest|real|actual) (opinion|take|thought))\b/i,
  /\b(can you really|do you actually|are you sure)\b/i,
  /\b(nobody knows|no one knows|unanswerable|impossible to)\b/i,
];
...
function applyGenuineAdmission(response, text) {
  const turn = mem.turn;
  if (turn < 5) return response; // need rapport first
  if (turn - lastAdmissionTurn < ADMISSION_COOLDOWN) return response;
  if (admissionCount >= MAX_ADMISSIONS_SESSION) return response;

  // Check if this is the kind of question that warrants genuine admission
  const isHardQuestion = HARD_QUESTION_SIGNALS.some(p => p.test(text));
  if (!isHardQuestion) return response;

  // 50% fire rate on hard questions — sometimes just answer confidently
  if (Math.random() > 0.50) return response;

  // Don't double up with epistemic hedging
  if (/\b(i think|maybe|perhaps|not sure|might be|could be)\b/i.test(response.slice(0, 40))) return response;

  lastAdmissionTurn = turn;
  admissionCount++;

  // Pick admission type based on question nature
  let type;
  if (/\b(should i|best way|worth|decide)\b/i.test(text)) type = "humble_take";
  else if (/\b(meaning|purpose|why do|point of)\b/i.test(text)) type = "shared_wonder";
  else type = "honest_doubt";

  const admission = pick(ADMISSION_TYPES[type]);
  const addFollowup = Math.random() < 0.45;
  const followup = addFollowup ? " " + pick(ADMISSION_FOLLOWUPS) : "";

  // For humble_take, prepend and keep response
  if (type === "humble_take") {
    return admission + " " + response + followup;
  }
  // For honest_doubt and shared_wonder, replace with admission + shortened response + followup
  const sentences = response.split(/(?<=[.!?])\s+/);
  const kept = sentences.slice(0, 2).join(" ");
  return admission + " " + kept + followup;
}

/* ═══ Round 127: Deferred Answer Recovery ═══
 * Track user questions across turns. When the AI's response doesn't address
 * one, stash it. On a later turn, naturally circle back: "Oh — you also asked..."
 * This mimics how real people catch dropped threads mid-conversation.
 */
let userQuestionQueue = []; // { q: string, turn: number, keywords: string[] }
let lastDeferredRecoverTurn = 0;
const DEFERRED_RECOVER_COOLDOWN = 5;
...
function extractUserQuestions(text) {
  // Split into sentences, find ones ending with ?
  const sentences = text.split(/(?<=[.!?])\s+|(?<=[.!?])$/);
  return sentences
    .filter(s => s.trim().endsWith("?"))
    .map(s => s.trim())
    .filter(s => s.length > 8 && s.split(/\s+/).length >= 3) // skip tiny fragments
    .filter(s => !/^(right|ok|yeah|really|huh|what|hmm)\??$/i.test(s)); // skip rhetorical
}

function getQuestionKeywords(q) {
  const stops = new Set(["what","how","why","when","where","who","which","does","did","do","is","are","was","were","can","could","would","should","will","the","a","an","to","of","in","for","it","you","i","my","your","this","that","about","have","has"]);
  return q.toLowerCase().replace(/[?!.,]/g, "").split(/\s+/).filter(w => w.length > 2 && !stops.has(w));
}

function wasQuestionAddressed(q, response) {
  const kws = getQuestionKeywords(q);
  if (kws.length === 0) return true; // can't tell, assume addressed
  const respLower = response.toLowerCase();
  const matched = kws.filter(w => respLower.includes(w));
  return matched.length / kws.length >= 0.4; // 40% keyword overlap = addressed
}

function trackUserQuestions(text) {
  const questions = extractUserQuestions(text);
  for (const q of questions) {
    const kws = getQuestionKeywords(q);
    // Don't add duplicates
    if (userQuestionQueue.some(uq => {
      const overlap = uq.keywords.filter(k => kws.includes(k));
      return overlap.length >= Math.min(2, kws.length);
    })) continue;
    userQuestionQueue.push({ q, turn: mem.turn, keywords: kws });
    if (userQuestionQueue.length > MAX_QUEUED_QUESTIONS) userQuestionQueue.shift();
  }
}

function pruneAnsweredQuestions(response) {
  userQuestionQueue = userQuestionQueue.filter(uq => !wasQuestionAddressed(uq.q, response));
}

function applyDeferredAnswerRecovery(response, text) {
  const turn = mem.turn;

  // First, track any new questions in this user message
  trackUserQuestions(text);

  // Don't recover too early or too often
  if (turn < 4) return response;
  if (turn - lastDeferredRecoverTurn < DEFERRED_RECOVER_COOLDOWN) return response;

  // Prune questions that this response already addresses
  pruneAnsweredQuestions(response);

  // Find a stale question (at least 2 turns old) to recover
  const stale = userQuestionQueue.find(uq => turn - uq.turn >= 2 && turn - uq.turn <= 12);
  if (!stale) return response;

  // 40% fire rate — don't always circle back
  if (Math.random() > 0.40) return response;

  // Build a recovery snippet
  const opener = pick(RECOVER_OPENERS);
  const shortQ = stale.q.length > 60 ? stale.q.slice(0, 57) + "..." : stale.q;

  // Generate a brief, topic-relevant acknowledgment
  const kw = stale.keywords[0] || "";
  const acks = [
    `about ${kw} — honestly I think`,
    `— "${shortQ}" — my take is`,
    `about that. Short answer:`,
    `— "${shortQ}" —`,
  ];
  const ack = pick(acks);

  lastDeferredRecoverTurn = turn;
...
function addGrounding(response, userText, parsed, sent, topics) {
  ...
  if (/^(gotcha|right|mm|okay|got it|makes sense|I hear|let me think)/i.test(response)) return response;
...
const CROSS_DOMAIN_ANALOGIES = {
  code: [
    "it's kinda like cooking — you follow a recipe but the best stuff happens when you wing it",
    "ok this is weirdly like jazz?? there's structure but the fun part is when you go off script",
    "it's literally gardening — plant stuff, water it, rip out what's dead, and eventually something grows lol",
    "it's like building a house — the foundation matters way more than the paint color but nobody sees the foundation",
    "debugging is just being a detective. clues, red herrings, and the bug is always the dumbest thing",
    "writing code is lowkey just writing — both are about being clear, just for different audiences",
  ],
  design: [
  ...
  work: [
    "working in a team is like being in a band — everyone has their thing and the magic is when you actually listen to each other",
    "deadlines are like water — they always find the cracks in your schedule lol",
    "managing a project is herding cats except the cats all have opinions about which direction to go",
    "it's a relay race thing — you can be the fastest but if the handoff is bad none of it matters",
  ],
  general: [
    "there's a chess thing here — sometimes the best move doesn't look like a move at all",
    "it's like navigating by stars — you can't reach them but they show you which way to go",
    "the iceberg thing — what people see is like 10% and the other 90% is work nobody talks about",
    "it's like rivers carving canyons — not by force, just persistence over time",
    "it's the map vs territory thing — the model is useful but it's never the whole picture",
    "there's a music thing here — a rest is as important as a note. the pause gives everything else meaning",
  ],
};
...
const DEPTH_TEMPLATES = {
  tech: {
    0: [ // Surface: getting to know
      t => `wait how'd you end up doing ${t}?`,
      t => `how long have you been messing with ${t}?`,
      t => `what's your ${t} setup like?`,
      t => `is ${t} a work thing or do you actually choose to do it lol`,
    ],
    1: [ // Engaged: specific experiences
      t => `ok what's the most annoying thing about ${t} tho`,
      t => `has ${t} ruined other tools for you or what`,
      t => `what's something about ${t} that caught you off guard`,
      t => `do you have like a go-to move in ${t}?`,
      t => `if you could fix one thing about ${t} what would it be`,
    ],
    2: [ // Deep: opinions and tradeoffs
      t => `ok real talk where does ${t} actually suck`,
      t => `if you started fresh tomorrow would you still go with ${t}?`,
      t => `what do people get wrong about ${t}`,
      t => `where do you think ${t} is headed`,
      t => `what's something about ${t} only people deep in it would know`,
    ],
    3: [ // Expert: nuanced debate-level
      t => `ok devil's advocate — what's the best argument against ${t}`,
      t => `if ${t} just vanished what would you miss and what would you not lol`,
      t => `is there like a deeper philosophy behind ${t} or is it just vibes`,
      t => `hot take time — unpopular ${t} opinion go`,
    ],
  },
  design: {
    0: [
      t => `what got you into ${t}?`,
      t => `is ${t} like your main thing or are you just exploring`,
      t => `what do you use for ${t}?`,
    ],
    1: [
      t => `what's the hardest part about ${t} honestly`,
      t => `has your ${t} approach changed or has it always been the same vibe`,
      t => `who does ${t} really well that you lowkey admire`,
      t => `what's something about ${t} that's underrated`,
    ],
    2: [
      t => `when does ${t} go from like good to overthinking it`,
      t => `what ${t} trend is gonna age terribly lol`,
      t => `how do you even know when ${t} is done tho`,
    ],
    3: [
      t => `ok but ${t} ideals vs actually shipping — how do you deal w that`,
      t => `if you could change one thing about how people do ${t} what would it be`,
    ],
  },
  life: {
    0: [
      t => `what's the best part about ${t}?`,
      t => `how'd you get into ${t}?`,
      t => `is ${t} a new thing or have you been into it for a while`,
    ],
    1: [
      t => `what's been your best ${t} experience so far`,
      t => `has ${t} connected you w any cool people`,
      t => `if someone wanted to try ${t} what would you tell them`,
    ],
    2: [
      t => `has ${t} changed how you think about other stuff`,
      t => `what do most people get wrong about ${t}`,
      t => `is there a specific ${t} moment that like stuck with you`,
    ],
    3: [
      t => `what does ${t} actually mean to you tho like beyond the surface`,
      t => `has ${t} changed something like fundamental about how you see things`,
    ],
  },
};
...
function deepenQuestions(response, topics) {
  ...
  const genericPatterns = [
    ...
    /What's your perspective on that\?$/i,
  ];
...
let vibeStreak = 0;
let vibeHistory = []; // track last N vibes for conversation-level vibe reading

// Get the dominant conversation vibe from recent history
function getConversationVibe() {
  if (vibeHistory.length < 3) return "neutral";
  const recent = vibeHistory.slice(-5);
  const counts = {};
  for (const v of recent) counts[v] = (counts[v] || 0) + 1;
  let best = "neutral", bestCount = 0;
  for (const [v, c] of Object.entries(counts)) {
    if (c > bestCount && v !== "neutral") { best = v; bestCount = c; }
  }
  // Only return if dominant vibe appears in 60%+ of recent messages
  return bestCount >= recent.length * 0.6 ? best : "neutral";
}
...
function detectVibe(text, sent, parsed) {
  ...
  prevVibe = best;
  vibeHistory.push(best);
  if (vibeHistory.length > 10) vibeHistory.shift();

  return { vibe: best, score: bestScore, scores, conversationVibe: getConversationVibe() };
}
...
function applyVibeContagion(response, text, sent, parsed) {
  ...
  // High fire rate — friends always vibe-match
  if (Math.random() > 0.75) return response; // 75% fire rate
  ...
  switch (vibe) {
    case "playful": {
      // Match their energy — loosen up, get silly
      if (!/haha|lol|lmao|😂|bruh/i.test(r)) {
        const swaps = [
          [/\bvery\b/i, "lowkey"], [/\breally\b/i, "actually"],
          [/\bgreat\b/i, "fire"], [/\binteresting\b/i, "wild"],
          [/\bgood\b/i, "solid"], [/\bI think\b/i, "i feel like"],
        ];
    ...
    case "serious": {
      // Drop the jokes, be real
      r = r.replace(/\b(?:lol|haha|lmao|heh|😂|💀)\b/gi, "").trim();
      r = r.replace(/!+/g, ".");
    ...
    case "introspective": {
      // Slow down, match their thoughtful energy
      if (Math.random() > 0.4) {
        const swaps = [
          [/^(I think)/i, "hmm i wonder if"], [/\bdefinitely\b/i, "maybe"],
          [/\bclearly\b/i, "it feels like"], [/\bshould\b/i, "could"],
        ];
        for (const [pat, rep] of swaps) {
    ...
    case "excited": {
      // MATCH THE HYPE
      const swaps = [
        [/\bgood\b/i, "so good"], [/\bnice\b/i, "sick"],
        [/\bcool\b/i, "SO COOL"], [/\binteresting\b/i, "insane"],
      ];
      for (const [pat, rep] of swaps) {
        if (pat.test(r) && Math.random() > 0.4) { r = r.replace(pat, rep); break; }
      }
      if (!/!/.test(r) && r.length > 15) r = r.replace(/\.(\s|$)/, "!$1");
      if (vibeStreak >= 2) r = r.replace(/^./, c => c.toUpperCase());
      break;
    }

    case "anxious": {
      // Be reassuring but like a friend, not a therapist
      if (!/it's ok|no worries|don't worry|you're fine|you're good/i.test(r)) {
        const reassure = [
          "hey no you're good. ",
        ...
        if (Math.random() > 0.4) {
          r = pick(reassure) + r.charAt(0).toLowerCase() + r.slice(1);
        }
      }
      break;
    }
    ...
    case "matter_of_fact": {
      // Be direct back — no fluff
      r = r.replace(/\b(?:I think|I guess|maybe|perhaps|kind of|sort of)\b,?\s*/gi, "");
      r = r.replace(/\s+/g, " ").trim();
      if (r.length > 120) {
        const sentences = r.match(/[^.!?]+[.!?]+/g);
        if (sentences && sentences.length > 2) r = sentences.slice(0, 2).join(" ").trim();
      }
      break;
    }
    ...
    case "creative": {
      ...
      const swaps = [
        [/\bgood\b/i, "dope"], [/\binteresting\b/i, "sick"],
        [/\bworks\b/i, "hits"], [/\bmakes sense\b/i, "clicks"],
      ];
      for (const [pat, rep] of swaps) {
        if (pat.test(r) && Math.random() > 0.4) { r = r.replace(pat, rep); break; }
      }
    ...
    case "vulnerable": {
      // Be gentle — like a real friend, not a counselor
      ...
      if (!/i hear you|that's real|i get it|that sucks|i'm sorry/i.test(r)) {
        ...
        if (Math.random() > 0.3) {
          r = pick(warmth) + r.charAt(0).toLowerCase() + r.slice(1);
        }
      }
      // Strip probing questions — don't interrogate someone being vulnerable
      if (/\?/.test(r)) {
        const sentences = r.match(/[^.!?]+[.!?]+/g);
        if (sentences && sentences.length >= 2) {
          r = sentences.filter(s => !/\?/.test(s)).join(" ").trim();
          if (r.length < 15) r = response;
        }
      }
      break;
    }
...
/* ── Vibe Mismatch Guard ──
 * Catches tone clashes: being goofy when they're upset, being stiff when
 * they're clearly joking. A friend never misreads the room this badly.
 */
function guardVibeMismatch(response, text, sent, parsed) {
  const { vibe } = detectVibe(text, sent, parsed);
  let r = response;

  // Don't be playful/funny when they're vulnerable or venting
  if ((vibe === "vulnerable" || vibe === "anxious") && /\b(lol|lmao|haha|😂|💀|LETS GO|fire)\b/i.test(r)) {
  ...
  if (vibe === "serious" && vibeStreak >= 2) {
    ...
    if (/^(haha|lol|lmao)/i.test(r)) r = r.replace(/^(haha|lol|lmao)\s*/i, "");
  }

  // Don't over-exclaim when they're introspective
  if (vibe === "introspective" && (r.match(/!/g) || []).length >= 2) {
...
function applyEchoBack(response, text, sent, topics) {
  ...
  if (/^(Oh |Hmm|Right|Wait|Gotcha|Got it|Makes sense|I hear|Actually|So |Yeah|Ooh)/i.test(response)) return response;
...
const PERSP_ACK_TEMPLATES = {
  ...
  stance: [
    "ooh ok i like that framing",
    "solid starting point — let me build off that",
    "yeah that's a good anchor actually",
  ],
  preference: [
    "fair — i get the appeal tbh",
    "ok yeah i can see why you'd lean that way. here's a thought though —",
  ],
  framing: [
...
const ENCOURAGEMENT_POOLS = {
  ...
  insight: [
    ...
    "that angle is more interesting than you think",
    "oh that's a good catch actually",
  ],
  growth: [
    "love that you're investing in yourself tbh",
    "that growth mindset tho >>",
    "the fact that you're even thinking about this? says a lot",
    "that self-awareness is lowkey rare",
    "yep — that's exactly how you level up",
    "most people don't actively work on themselves like that ngl",
  ],
};
...
export async function getAIResponse(input) {
  ...
  // ═══ Spam detection: catch rapid repeated messages ═══
  const lastUserMsg = mem.lastUser();
  if (lastUserMsg && lastUserMsg.text.toLowerCase() === text.toLowerCase()) {
    _spamCount = (_spamCount || 0) + 1;
    if (_spamCount >= 3) {
      _spamCount = 0;
      const spamResps = ["ok i hear you lol, you don't need to say it three times","lol you really want my attention huh","message received, loud and clear 😂","i got it the first time but i appreciate the enthusiasm"];
      return { text: pickNew(spamResps), typingMs: 500, pause: null };
    }
  } else {
    _spamCount = 0;
  }
  ...
  if (alphaChars.length >= 4 && alphaChars === alphaChars.toUpperCase() && /[A-Z]{4,}/.test(text)) {
    // Don't override — just flag it so the response pipeline can match energy
    // But if it's a very short all-caps message that won't match intents well, handle directly
    if (text.length <= 30 && !/greeting|farewell|howAreYou/.test(mem.lastIntent || "")) {
      ...
      if (Math.random() > 0.6) {
        return { text: pickNew(capsResps), typingMs: 400, pause: null };
      }
    }
  }
  ...
  // ═══ Vibe mismatch guard: catch tone clashes (goofy when they're sad, stiff when they're joking) ═══
  response = guardVibeMismatch(response, text, sent, parsed);
  ...
  // ═══ Socratic questioning: probe user claims with warm curiosity ═══
  response = applySocraticQuestioning(response, text);

  // ═══ Meta-humor: rare self-aware chatbot jokes for delight ═══
  response = applyMetaHumor(response, text);

  // ═══ Progressive disclosure: teaser + invite for complex topics ═══
  response = applyProgressiveDisclosure(response, text, currentTopics);
  ...
  // ═══ Micro-validation: tiny affirmations to signal active listening ═══
  response = applyMicroValidation(response, text);
  ...
  // ═══ Breakthrough celebration: mirror the user's aha moments ═══
  response = applyBreakthroughCelebration(response, text);

  // ═══ User profiling: build & occasionally share personality insights ═══
  updateUserProfile(text);
  response = applyUserProfiling(response, text);

  // ═══ Conversational pacing: mirror user's message rhythm ═══
  response = applyConversationalPacing(response, text);

  // ═══ Ambiguity detection: clarify vague inputs instead of guessing ═══
  response = applyAmbiguityDetection(response, text);

  // ═══ Genuine admission: honest uncertainty on hard questions ═══
  response = applyGenuineAdmission(response, text);

  // ═══ Deferred answer recovery: circle back to user's unanswered questions ═══
  response = applyDeferredAnswerRecovery(response, text);
