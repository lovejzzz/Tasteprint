...
function C({type,v=0,p,editable,texts={},onText,font=0}){
  ...
  if(type==="navbar"){
    ...
    return <div style={{...b,background:"#0a0a0c",borderRadius:0,borderBottom:`1px solid ${p.ac}20`,padding:"0 18px",display:"flex",alignItems:"center",justifyContent:"space-between",fontFamily:"'JetBrains Mono',monospace"}}><div style={{display:"flex",alignItems:"center",gap:16}}><T k="brand" s={{fontSize:11,fontWeight:600,color:p.ac,letterSpacing:"0.06em"}}>SYS.ID</T>{["HOME","DROPS","TERMINAL"].map((t,i)=><T key={i} k={`link${i}`} s={{fontSize:10,color:i===0?"#ccc":"#555",fontWeight:400,letterSpacing:"0.04em"}}>{t}</T>)}</div><div style={{height:28,width:28,borderRadius:2,border:`1px solid ${p.ac}30`,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:10,color:p.ac,opacity:.6,animation:"tp-pulse 2.5s ease infinite"}}>⌘</span></div></div>;
  }
  ...
  if(type==="stat-card"){
    ...
    if(v===3)return <div style={{...b,background:p.ac,borderRadius:14,padding:14,display:"flex",flexDirection:"column",justifyContent:"space-between"}}><T k="label" s={{fontSize:10,color:"#ffffff88",fontWeight:500}}>Revenue</T><T k="value" s={{fontSize:26,fontWeight:600,color:"#fff"}}>$48.2k</T><T k="delta" s={{fontSize:11,color:"#ffffff99"}}>+12.5%</T></div>;
  ...
  /* ---- TOGGLE ---- */
  if(type==="toggle"){
    if(v===0)return <div style={{...b,background:p.ac,borderRadius:999,padding:3,display:"flex",alignItems:"center",justifyContent:"flex-end",transition:"background .3s"}}><div style={{width:20,height:20,borderRadius:999,background:"#fff",boxShadow:"0 1px 3px rgba(0,0,0,.1)",transition:"transform .3s cubic-bezier(.4,.2,.2,1)"}}/></div>;
    return <div style={{...b,background:p.ac+"2A",borderRadius:6,padding:3,display:"flex",alignItems:"center",justifyContent:"flex-end",transition:"background .3s"}}><div style={{width:20,height:20,borderRadius:4,background:p.ac,transition:"transform .3s cubic-bezier(.4,.2,.2,1)"}}/></div>;
  }
  ...
  if(type==="toast"){
    ...
    if(v===1)return <div style={{...b,background:p.tx,borderRadius:8,padding:"0 14px",display:"flex",alignItems:"center",gap:10,animation:"tp-slidein .4s ease-out"}}><T k="label" s={{fontSize:12,color:p.bg,fontWeight:500}}>Changes saved</T><T k="action" s={{marginLeft:"auto",fontSize:11,color:p.bg,opacity:.45}}>Undo</T></div>;
  ...
  if(type==="progress"){
    const rd=[999,4,0];
    return <div style={{...b,background:p.su,borderRadius:rd[v],overflow:"hidden"}}><div style={{width:"68%",height:"100%",background:v===1?`linear-gradient(90deg,${p.ac},${p.ac2})`:p.ac,borderRadius:rd[v],animation:"tp-fill .8s ease-out forwards"}}/></div>;
  }
  ...
  if(type==="sidebar"){
    ...
    return <div style={{...b,background:`${p.card}55`,backdropFilter:"blur(20px)",borderRadius:16,border:`1px solid ${p.ac}12`,padding:12,display:"flex",flexDirection:"column",gap:2}}><T k="brand" s={{fontSize:13,fontWeight:600,color:p.tx,padding:"2px 12px 10px"}}>Workspace</T>{items.map((t,i)=><div key={i} style={{padding:"8px 12px",borderRadius:10,background:i===0?`${p.ac}12`:"transparent",display:"flex",alignItems:"center",gap:10}}><div style={{width:16,height:16,borderRadius:6,background:i===0?`${p.ac}35`:p.mu+"15",flexShrink:0}}/><T k={`nav${i}`} s={{fontSize:12,color:i===0?p.tx:p.mu,fontWeight:i===0?500:400}}>{t}</T></div>)}</div>;
  }
  ...
  if(type==="promo-banner"){
    if(v===0)return <div style={{...b,background:`linear-gradient(135deg,${p.ac},${p.ac2},${p.ac})`,backgroundSize:"200% 200%",animation:"tp-gradient-shift 3s ease infinite",borderRadius:12,padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between"}}><div style={{display:"flex",alignItems:"center",gap:12}}><T k="text" s={{fontSize:13,fontWeight:600,color:"#fff"}}>Summer Sale — 30% off</T><div style={{background:"#fff3",borderRadius:6,padding:"3px 10px"}}><T k="code" s={{fontSize:11,fontWeight:600,color:"#fff",letterSpacing:"0.06em"}}>SUMMER30</T></div></div><div style={{height:32,width:80,borderRadius:8,background:"#fff",display:"flex",alignItems:"center",justifyContent:"center"}}><T k="cta" s={{color:p.ac,fontSize:11,fontWeight:600}}>Shop now</T></div></div>;
  ...
  if(type==="dash-panel"){
    const bars=[65,42,78,35,55,88,48];
    ...
    if(v===1)return <div style={{...b,background:"#0c0c0e",borderRadius:4,border:`1px solid ${p.ac}20`,padding:14,display:"flex",flexDirection:"column",gap:10,fontFamily:"'JetBrains Mono',monospace"}}><T k="title" s={{fontSize:10,color:p.ac,fontWeight:600,letterSpacing:"0.06em"}}>SIGNAL CAPTURE</T><div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{position:"relative",width:90,height:90}}><svg width="90" height="90" viewBox="0 0 90 90"><circle cx="45" cy="45" r="38" fill="none" stroke={p.ac+"15"} strokeWidth="4"/><circle cx="45" cy="45" r="38" fill="none" stroke={p.ac} strokeWidth="4" strokeDasharray={`${0.72*238} ${238}`} strokeLinecap="round" transform="rotate(-90 45 45)" style={{animation:"tp-ring 1s ease-out forwards"}}/></svg><div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><T k="value" s={{fontSize:18,fontWeight:600,color:p.ac}}>72%</T><T k="sub" s={{fontSize:8,color:"#555"}}>SIGNAL</T></div></div></div><div style={{display:"flex",gap:8}}>{[{l:"IN",v:"1.2K"},{l:"OUT",v:"847"},{l:"ERR",v:"3"}].map((m,i)=><div key={i} style={{flex:1,padding:"4px 0",borderTop:`1px solid ${p.ac}15`,textAlign:"center"}}><div style={{fontSize:7,color:"#555",letterSpacing:"0.08em"}}>{m.l}</div><T k={`m${i}`} s={{fontSize:10,color:i===2?"#E05050":p.ac,opacity:.8}}>{m.v}</T></div>)}</div></div>;
  ...
  if(type==="chat"){
    ...
    if(v===0)return <div style={{...b,background:p.card,borderRadius:14,border:`1px solid ${p.bd}`,padding:14,display:"flex",flexDirection:"column",gap:8,overflow:"auto"}}>{msgs.map((m,i)=><div key={i} style={{display:"flex",justifyContent:m.from==="me"?"flex-end":"flex-start",animation:`tp-slidein .3s ease-out ${i*0.1}s both`}}><div style={{maxWidth:"75%",padding:"8px 12px",borderRadius:m.from==="me"?"12px 12px 4px 12px":"12px 12px 12px 4px",background:m.from==="me"?p.ac:p.su}}><T k={`m${i}`} s={{fontSize:11,color:m.from==="me"?"#fff":p.tx,lineHeight:1.4}}>{m.text}</T></div></div>)}<div style={{display:"flex",gap:6,marginTop:"auto"}}><div style={{flex:1,height:32,borderRadius:999,background:p.su,border:`1px solid ${p.bd}`,padding:"0 12px",display:"flex",alignItems:"center"}}><T k="input" s={{fontSize:11,color:p.mu,opacity:.4}}>Type a message...</T></div><div style={{width:32,height:32,borderRadius:999,background:p.ac,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{color:"#fff",fontSize:12}}>↑</span></div></div></div>;
  ...
  if(type==="notification"){
    const items=[{title:"New comment",desc:"Alex replied to your post",time:"2m",read:false},{title:"Deployment done",desc:"Production build succeeded",time:"1h",read:false},{title:"Weekly report",desc:"Your stats are ready",time:"3h",read:true}];
    if(v===0)return <div style={{...b,background:p.card,borderRadius:14,border:`1px solid ${p.bd}`,overflow:"hidden",display:"flex",flexDirection:"column"}}><div style={{padding:"10px 14px",borderBottom:`1px solid ${p.bd}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}><T k="title" s={{fontSize:12,fontWeight:600,color:p.tx}}>Notifications</T><div style={{background:p.ac,borderRadius:999,padding:"1px 6px"}}><span style={{fontSize:9,color:"#fff",fontWeight:600}}>2</span></div></div>{items.map((it,i)=><div key={i} style={{padding:"10px 14px",borderBottom:i<2?`1px solid ${p.bd}`:"none",display:"flex",gap:10,alignItems:"flex-start",background:it.read?"transparent":p.ac+"06",animation:`tp-slidein .3s ease-out ${i*0.08}s both`}}>{!it.read&&<div style={{width:6,height:6,borderRadius:999,background:p.ac,marginTop:5,flexShrink:0,animation:"tp-pulse 2s ease infinite"}}/>}<div style={{flex:1}}><T k={`n${i}`} s={{fontSize:11,fontWeight:it.read?400:500,color:p.tx}}>{it.title}</T><T k={`d${i}`} s={{fontSize:10,color:p.mu,display:"block"}}>{it.desc}</T></div><span style={{fontSize:9,color:p.mu,opacity:.5}}>{it.time}</span></div>)}</div>;
    if(v===1)return <div style={{...b,display:"flex",flexDirection:"column",gap:6}}>{items.map((it,i)=><div key={i} style={{display:"flex",gap:8,alignItems:"center",padding:"6px 0",borderBottom:i<2?`1px solid ${p.bd}`:"none",animation:`tp-fadein .3s ease-out ${i*0.1}s both`}}><div style={{width:28,height:28,borderRadius:8,background:it.read?p.su:p.ac+"14",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontSize:12,opacity:.5}}>{i===0?"💬":i===1?"🚀":"📊"}</span></div><div style={{flex:1}}><T k={`n${i}`} s={{fontSize:11,fontWeight:500,color:p.tx}}>{it.title}</T></div><span style={{fontSize:9,color:p.mu}}>{it.time}</span></div>)}</div>;
    return <div style={{...b,display:"flex",flexDirection:"column",gap:4}}>{items.map((it,i)=><div key={i} style={{display:"flex",gap:8,alignItems:"center",animation:`tp-fadein .3s ease-out ${i*0.08}s both`}}><div style={{width:6,height:6,borderRadius:999,background:it.read?p.mu+"20":p.ac,flexShrink:0}}/><T k={`n${i}`} s={{fontSize:11,color:it.read?p.mu:p.tx,fontWeight:it.read?400:500}}>{it.title}</T><span style={{marginLeft:"auto",fontSize:9,color:p.mu,opacity:.4}}>{it.time}</span></div>)}</div>;
  }
