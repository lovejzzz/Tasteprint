...
function C({type,v=0,p,editable,texts={},onText,font=0}){
  ...
  if(type==="navbar"){
    ...
    return <div style={{...b,background:"#0a0a0c",borderRadius:0,borderBottom:`1px solid ${p.ac}20`,padding:"0 18px",display:"flex",alignItems:"center",justifyContent:"space-between",fontFamily:"'JetBrains Mono',monospace"}}><div style={{display:"flex",alignItems:"center",gap:16}}><T k="brand" s={{fontSize:11,fontWeight:600,color:p.ac,letterSpacing:"0.06em"}}>SYS.ID</T>{["HOME","DROPS","TERMINAL"].map((t,i)=><T key={i} k={`link${i}`} s={{fontSize:10,color:i===0?"#ccc":"#555",fontWeight:400,letterSpacing:"0.04em"}}>{t}</T>)}</div><div style={{height:28,width:28,borderRadius:2,border:`1px solid ${p.ac}30`,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:10,color:p.ac,opacity:.6}}>⌘</span></div></div>;
  }
  ...
  if(type==="dash-panel"){
    ...
    if(v===0)return <div style={{...b,background:"#0c0c0e",borderRadius:4,border:`1px solid ${p.ac}20`,padding:14,display:"flex",flexDirection:"column",gap:8,fontFamily:"'JetBrains Mono',monospace"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><T k="title" s={{fontSize:10,color:p.ac,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.08em"}}>TELEMETRY</T><div style={{display:"flex",alignItems:"center",gap:4}}><div style={{width:5,height:5,borderRadius:999,background:"#4CAF50"}}/><T k="status" s={{fontSize:8,color:"#4CAF50",letterSpacing:"0.05em"}}>LIVE</T></div></div><div style={{display:"flex",gap:3,alignItems:"flex-end",flex:1,padding:"8px 0"}}>{bars.map((h,i)=><div key={i} style={{flex:1,background:p.ac,opacity:i===bars.length-1?.8:.3,borderRadius:1,height:`${h}%`}}/>)}</div><div style={{display:"flex",justifyContent:"space-between"}}><T k="metric1" s={{fontSize:9,color:"#666"}}>PKT: 2,847</T><T k="metric2" s={{fontSize:9,color:"#666"}}>LAT: 12ms</T></div></div>;
    if(v===1)return <div style={{...b,background:"#0c0c0e",borderRadius:4,border:`1px solid ${p.ac}20`,padding:14,display:"flex",flexDirection:"column",gap:10,fontFamily:"'JetBrains Mono',monospace"}}><T k="title" s={{fontSize:10,color:p.ac,fontWeight:600,letterSpacing:"0.06em"}}>SIGNAL CAPTURE</T><div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{position:"relative",width:90,height:90}}><svg width="90" height="90" viewBox="0 0 90 90"><circle cx="45" cy="45" r="38" fill="none" stroke={p.ac+"15"} strokeWidth="4"/><circle cx="45" cy="45" r="38" fill="none" stroke={p.ac} strokeWidth="4" strokeDasharray={`${0.72*238} ${238}`} strokeLinecap="round" transform="rotate(-90 45 45)"/></svg><div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><T k="value" s={{fontSize:18,fontWeight:600,color:p.ac}}>72%</T><T k="sub" s={{fontSize:8,color:"#555"}}>SIGNAL</T></div></div></div><div style={{display:"flex",gap:8}}>{[{l:"IN",v:"1.2K"},{l:"OUT",v:"847"},{l:"ERR",v:"3"}].map((m,i)=><div key={i} style={{flex:1,padding:"4px 0",borderTop:`1px solid ${p.ac}15`,textAlign:"center"}}><div style={{fontSize:7,color:"#555",letterSpacing:"0.08em"}}>{m.l}</div><T k={`m${i}`} s={{fontSize:10,color:i===2?"#E05050":p.ac,opacity:.8}}>{m.v}</T></div>)}</div></div>;
    return <div style={{...b,background:"#0c0c0e",borderRadius:4,border:`1px solid ${p.ac}15`,padding:14,display:"flex",flexDirection:"column",gap:8,fontFamily:"'JetBrains Mono',monospace"}}><T k="title" s={{fontSize:10,color:"#888",fontWeight:500}}>System</T><T k="value" s={{fontSize:28,fontWeight:600,color:p.ac,letterSpacing:"-0.02em"}}>847ms</T><div style={{flex:1,display:"flex",alignItems:"flex-end"}}><svg style={{width:"100%",height:"100%"}} viewBox="0 0 100 40" preserveAspectRatio="none"><path d="M0,35 Q10,30 20,28 T40,20 T60,25 T80,15 T100,18" fill="none" stroke={p.ac+"50"} strokeWidth="1.5"/><path d="M0,35 Q10,30 20,28 T40,20 T60,25 T80,15 T100,18 L100,40 L0,40 Z" fill={p.ac+"08"}/></svg></div><T k="sub" s={{fontSize:9,color:"#555"}}>avg response · last 24h</T></div>;
  }
