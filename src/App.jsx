...
const HAS_TEXT=new Set(["button","card","card-sm","hero","navbar","tabs","heading","stat-card","input","search","badge","toast","modal","sidebar","footer","accordion","table","list-item","dropdown","select","checkbox","alert","pagination","pricing-card","tooltip","breadcrumb","testimonial","cmd-palette","stepper","timeline"]);
...
function C({type,v=0,p,editable,texts={},onText,font=0}){
  ...
  if(type==="card-sm"){
    ...
    return <div style={{...b,background:p.su,borderRadius:16,padding:14,display:"flex",flexDirection:"column",gap:6}}><div style={{width:30,height:30,borderRadius:999,background:p.ac+"1A"}}/><T k="title" s={{fontSize:12,fontWeight:600,color:p.tx}}>Small card</T><T k="desc" s={{fontSize:10,color:p.mu}}>Short description</T></div>;
  }
  ...
  if(type==="modal"){
    ...
    if(v===1)return <div style={{...b,background:p.card,borderRadius:12,border:`1px solid ${p.bd}`,padding:24,display:"flex",flexDirection:"column",gap:12,alignItems:"center",textAlign:"center"}}><div style={{width:40,height:40,borderRadius:999,background:p.ac+"14",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:18,color:p.ac}}>?</span></div><T k="title" s={{fontSize:15,fontWeight:600,color:p.tx}}>Are you sure?</T><T k="body" s={{fontSize:11,color:p.mu,lineHeight:1.5}}>This cannot be undone.</T><div style={{display:"flex",gap:8}}><div style={{height:32,width:80,borderRadius:8,border:`1px solid ${p.bd}`,display:"flex",alignItems:"center",justifyContent:"center"}}><T k="cancel" s={{fontSize:11,color:p.mu}}>Cancel</T></div><div style={{height:32,width:80,borderRadius:8,background:p.ac,display:"flex",alignItems:"center",justifyContent:"center"}}><T k="confirm" s={{fontSize:11,color:"#fff",fontWeight:500}}>Confirm</T></div></div></div>;
